...
class MdnsDiscovery {
  constructor({ log, serviceTypes, hostnames, onDevice }) {
    this.log = log;
    this.serviceTypes = serviceTypes || ['_http._tcp.local', '_awair._tcp.local'];
  ...
  query() { for (const type of this.serviceTypes) this.socket?.send(dnsQuery(type), 5353, '224.0.0.251'); }
  handle(message) {
    for (const record of parseRecords(message)) {
      ...
      if (record.type === 33 && (/awair/i.test(record.name) || /awair/i.test(record.target))) this.verify(record.target.replace(/\.$/, ''), record.port);
    }
  }
...
function dnsQuery(name) {
  ...
  return Buffer.concat([header, encodeName(name), Buffer.from([0, 12, 0, 1])]);
}
...
// Keep parsing deliberately small: address and SRV records are all discovery needs.
function parseRecords(message) {
  try {
    ...
    for (let i = 0; i < count; i++) { let name; [name, offset] = readName(message, offset); const type = message.readUInt16BE(offset); const length = message.readUInt16BE(offset + 8); const start = offset + 10; offset = start + length; if (type === 33) { const port = message.readUInt16BE(start + 4); const [target] = readName(message, start + 6); records.push({ name, type, port, target }); } else records.push({ name, type }); }
    return records;
  } catch (_) { return []; }
}
function readName(message, offset, seen = new Set()) { const labels = []; let cursor = offset; let next = offset; while (cursor < message.length) { const length = message[cursor]; if (!length) { if (next === offset) next = cursor + 1; break; } if ((length & 0xc0) === 0xc0) { const pointer = ((length & 0x3f) << 8) | message[cursor + 1]; if (!seen.has(pointer)) { seen.add(pointer); const [suffix] = readName(message, pointer, seen); labels.push(suffix); } if (next === offset) next = cursor + 2; break; } labels.push(message.subarray(cursor + 1, cursor + 1 + length).toString()); cursor += length + 1; next = cursor; } return [labels.filter(Boolean).join('.'), next]; }
