...
All prior device settings are available inside `devices`: `ip`, `url`, `model`/`devType`, `manufacturer`, `serial`, `version`, `carbonDioxideThreshold`, `carbonDioxideThresholdOff`, `voc_mixture_mw`, `air_quality_method`, `polling_interval`, `limit`, `logging`, and `logging_level`. `configUrl`/`config_url` and `requestTimeout` are also supported. Defaults remain 10 seconds, `awair-score`, 1 data point, and a VOC molecular weight of 72.6657827301974.

## Discovery

Discovery is on by default. It browses `_http._tcp.local` and `_awair._tcp.local`, then only adds a service when its `/settings/config/data` response identifies an `awair-*` device. Discovered accessories are cached by device UUID (or MAC address), so they survive restarts and receive address updates.
